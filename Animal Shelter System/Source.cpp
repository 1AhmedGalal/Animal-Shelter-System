		...
		case 1:
			...
			for (int i = 0; i <= reportsNum; i++)
			{
				if (report[i].isConfirmed == 1 && report[i].isAdopted == false) {
					cout << count + 1 << "- " << left << setw(16) << report[i].type << left << setw(16) << report[i].location << left << setw(35) << report[i].health_situation << left << setw(45) << report[i].description << "\n";
					count++;
				}
				if (count != 0)
					cout << endl << "---------------------------------------------------------------------------------------------------------------------\n";
		...
		while (rightchoice == false)
		{
			...
			case 1:
				...
				for (int i = 0; i <= reportsNum; i++)
				{
					...
					{
						c++;
						cout << c << "- " << left << setw(16) << report[i].type << left << setw(16) << report[i].location << left << setw(35) << report[i].health_situation << left << setw(45) << report[i].description << "\n";
					}
				}
				if (c == 0) {
					SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 3);
			...
			case 2:
				...
				for (int i = 0; i <= reportsNum; i++)
				{
					if (report[i].report_user_idx == idxOfUser)
					{
						cout << c + 1 << "- " << left << setw(16) << report[i].type << left << setw(16) << report[i].location << left << setw(35) << report[i].health_situation << left << setw(45) << report[i].description << "\n";
						c++;
					}
				}
				if (c == 0) {
...
bool UsernameSignUpCheck()
{
	...
	while (true)
	{
		...
		int specialChar = tempUsername.find_first_of("!@#?><}{:+-/&$%×÷º");
...
bool NewUsernameCheck()
{
	...
	while (true)
	{
		...
		int specialChar = newUsername.find_first_of("!@#?><}{:+-/&$%×÷º");
...
void displayDonations_user()
{
	cout << "\n ALL THE MONEY YOU HAVE DONATED<3\n\n";
	cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n\n";
	cout << " TOTAL MONEY DONATED : " << totalDonations << '\n';
	cout << "\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n\n";
	system("pause");
